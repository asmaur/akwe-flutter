import 'package:akwe/src/data/services/budget_service.dart';
import 'package:akwe/src/exceptions/network_exceptions.dart';
import 'package:akwe/src/models/budgets/budget.dart';
import 'package:akwe/src/ui/shared/dialog_helper.dart';
import 'package:akwe/src/utils/status_code.dart';
import 'package:get/get.dart';
import 'package:month_year_picker/month_year_picker.dart';
import 'package:dio/dio.dart' as dio;
import 'package:akwe/src/translations/translation_keys.dart' as translation;



class BudgetAllPageController extends GetxController{
  final _selected = DateTime.now().obs;
  final budgetService = BudgetService();
  // final premiumService = Get.find<AppPremiumService>();
  final _service = BudgetService();
  var budgetItems = <Budget>[].obs;
  var isLoading = true.obs;


  @override
  void onInit() {
    getBudgetList();
    super.onInit();
  }

  getBudgetList() async{
    try{
      dio.Response response = await budgetService.getBudgetLast();
      if(response.statusCode == StatusCode.OK){
        var items = response.data;
        budgetItems.clear();
        items.forEach((item) => budgetItems.add(Budget.fromJson(item)));
        isLoading.value = false;
      }
    } on dio.DioException catch(e){
      isLoading.value = false;
      final errorMessage = DioExceptions.fromDioError(e);
      DialogHelper.showErrorDialog(title: translation.appMessageError.tr, description: errorMessage.message);
    }
  }

  selectMonthDialog() async {
    //final localeObj = locale != null ? Locale(locale) : null;
    final selected = await showMonthYearPicker(
      context: Get.context!,
      initialDate: _selected.value,
      firstDate: DateTime(2019),
      lastDate: DateTime(2030),
      locale: Get.locale,
    );

    if (selected != null) {
        _selected.value = selected;
        //print(_selected.value);
        getFilteredBudget();
    }
  }

  getFilteredBudget() async{
    DialogHelper.showLoading();
    try{

      dio.Response response = await _service.filterBudget(_selected.value.toIso8601String());

      if(response.statusCode == StatusCode.OK){
        var items = response.data;
        budgetItems.clear();
        items.forEach((item) => budgetItems.add(Budget.fromJson(item)));

      }

      DialogHelper.hideLoading();

    } on dio.DioException catch(e){
      DialogHelper.hideLoading();
      final errorMessage = DioExceptions.fromDioError(e);
      DialogHelper.showErrorDialog(title: translation.appMessageError.tr, description: errorMessage.message);
    }
  }

}